 ...
 */
function astronomyHorizontalToOurs(hor: Astronomy.HorizontalCoordinates): HorizontalCoords {
  return {
...
export function getJupiterMoons(
  ...
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const offsetScale = separationDeg * 0.1; // Scale factor for visualization
...
export function getMarsMoons(
  ...
  try {
    ...
    const moonData = [
      ...
      { name: 'Deimos', separationArcsec: 30, magnitude: 12.4, angle: Math.PI / 2 },
    ];
    
    for (const moon of moonData) {
      const separationDeg = moon.separationArcsec / 3600;
      const offsetScale = separationDeg * 0.1;
...
export function getSaturnMoons(
  ...
  try {
    ...
    const moonData = [
      ...
      { name: 'Enceladus', separationArcsec: 4, magnitude: 11.7, angle: Math.PI / 6 },
      { name: 'Tethys', separationArcsec: 5, magnitude: 10.2, angle: Math.PI / 3 },
      { name: 'Dione', separationArcsec: 6, magnitude: 10.4, angle: Math.PI / 2 },
      { name: 'Rhea', separationArcsec: 8, magnitude: 9.7, angle: (2 * Math.PI) / 3 },
      { name: 'Titan', separationArcsec: 22, magnitude: 8.3, angle: (5 * Math.PI) / 6 },
      { name: 'Iapetus', separationArcsec: 60, magnitude: 10.2, angle: Math.PI },
    ];
...
export function getUranusMoons(
  ...
  try {
    ...
    const moonData = [
      ...
      { name: 'Ariel', separationArcsec: 4, magnitude: 14.2, angle: Math.PI / 5 },
      { name: 'Umbriel', separationArcsec: 5, magnitude: 14.8, angle: (2 * Math.PI) / 5 },
      { name: 'Titania', separationArcsec: 8, magnitude: 13.7, angle: (3 * Math.PI) / 5 },
      { name: 'Oberon', separationArcsec: 10, magnitude: 13.9, angle: (4 * Math.PI) / 5 },
    ];
...
export function getNeptuneMoons(
  ...
  try {
    ...
    const moonData = [
      ...
      { name: 'Nereid', separationArcsec: 220, magnitude: 18.7, angle: Math.PI / 2 },
    ];
...
export function getPlutoMoons(
  ...
  try {
    ...
    const moonData = [
      ...
      { name: 'Hydra', separationArcsec: 2.5, magnitude: 23.3, angle: Math.PI / 2 },
      { name: 'Kerberos', separationArcsec: 1.5, magnitude: 26.1, angle: (3 * Math.PI) / 4 },
...
export function getAllCelestialBodies(
  ...
  const planets = bodies.map(name => getCelestialBodyPosition(name, location, dateTime));
